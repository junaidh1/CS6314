...
.then(response => {
  ...
  if (!response.ok) {
    throw new Error(`Failed to retrieve product IDs: ${response.status}`);
  }
  ...
  productIds.cart.forEach(prodId => {
    ...
    fetch(productInfoUrl)
      .then(infoResponse => {
        ...
        if (!infoResponse.ok) {
          throw new Error(`Failed to retrieve information for product ID ${prodId}: ${infoResponse.status}`);
        }
      ...
      .then(productInfo => {
        ...
        console.log(`Product ID: ${prodId.product_id}, Product Information:`, productInfo.product);
        ...
        cell5.innerHTML = Math.round(prodId.quantity * productInfo.product.price*100)/100;
        ...
        document.getElementById("final-total").innerHTML = Math.round(sum*100)/100
        document.getElementById("final-subtotal").innerHTML = Math.round(sum*100)/100
